...
if (!class_exists('Exopite_Simple_Options_Framework')) :
    ...
    class Exopite_Simple_Options_Framework
    {
        ...
        protected function define_menu_hooks()
        {
            ...
            do_action('tt_after_theme_option_save');
        ...
        public function plugin_action_links($links)
        {
            ...
            if (is_bool($this->config['settings_link'])) {
                ...
                if ($this->config['settings_link']) {
                    ...
                    $options_base_file_name = sanitize_file_name('admin.php');

                    $options_page_id = $this->config['parent'];

                    $settings_link = "{$options_base_file_name}?page={$options_page_id}";

                    $settings_link_array = array(
                        '<a href="' . admin_url($settings_link) . '">' . __('Authorization', $this->textdomain) . '</a>',
                    );
            ...
            if (is_array($this->config['settings_link'])) {
                ...
                foreach ($settings_links_config_array as $link) {
                    ...
                    $link_text = isset($link['text']) ? sanitize_text_field($link['text']) : __('Settings',  $this->textdomain);
...
//				$this->config = wp_parse_args( $this->config, $default );
        ...
        public function add_field($field, $value = null)
        {
            ...
            if (class_exists($class)) {
                ...
                $output .= esc_attr__('ERROR:', $this->textdomain) . ' ';
                $output .= esc_attr__('This field class is not available!', $this->textdomain);
        ...
        public function get_value($options, $field)
        {
             ...
             *
             *       if ( $unique == $this->triage_trak && $field['id'] == 'your-field-id' ) {
             *           // do the magic ;)
             *       }
             *
             *       return $value;
        ...
        public function display_options_page_header()
        {
            ...
            echo '<form method="post" action="options.php" enctype="multipart/form-data" name="' . $this->unique . '" class="exopite-sof-form-js ' . $this->unique . '-form" data-save="' . esc_attr__('Saving...', $this->textdomain) . '" data-saved="' . esc_attr__('Saved Successfully.', $this->textdomain) . '">';
            ...
            submit_button(esc_attr__('Save Settings', $this->textdomain), 'primary ' . 'exopite-sof-submit-button-js', $this->unique . '-save', false, array());
            echo '</fieldset>';
            echo '</header>';
        ...
        public function display_options_page_footer()
        {
            ...
            echo '<fieldset><span class="exopite-sof-ajax-message"></span>';
            submit_button(esc_attr__('Save Settings', $this->textdomain), 'primary ' . 'exopite-sof-submit-button-js', '', false, array());
            echo '</fieldset>';
